...
public String loginByAlipay(HttpServletRequest request1,HttpSession session) throws UnsupportedEncodingException, AlipayApiException {
     //使用auth_code换取接口access_token及用户userId
   	 AlipayClient alipayClient = new DefaultAlipayClient(AlipayConfig.gatewayUrl, AlipayConfig.app_id, AlipayConfig.merchant_private_key, AlipayConfig.format, AlipayConfig.charset, AlipayConfig.alipay_public_key, AlipayConfig.sign_type);
     AlipaySystemOauthTokenRequest request = new AlipaySystemOauthTokenRequest();
 	 request.setGrantType("authorization_code");
 	 request.setCode(request1.getParameter("auth_code"));
     ...
     if(userinfoShareResponse.isSuccess()){
    	 System.out.println(userinfoShareResponse.getMobile());
    	 //false--代表新用户。 数据库插入操作
    	 if(asi.isNewUser4Alipay(userinfoShareResponse.getUserId())==false) {
    		 ...
    		 session.setAttribute("userId", asi.findUserId(userinfoShareResponse.getUserId()));
    	 }else {
...
//	goodsDetail.put("goods_id", courseId);
//	goodsDetail.put("goods_name", courseId);
//	goodsDetail.put("quantity", courseId);
//	goodsDetail.put("price", courseId);
